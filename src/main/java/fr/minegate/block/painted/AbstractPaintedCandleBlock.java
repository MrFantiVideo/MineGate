...
public abstract class AbstractPaintedCandleBlock extends PaintedBlock
{
    public static final int             field_30987 = 3;
    public static final BooleanProperty LIT;

    protected AbstractPaintedCandleBlock(AbstractBlock.Settings settings)
    {
        super(settings);
    }
    ...
    public static void extinguish(@Nullable PlayerEntity player, BlockState state, WorldAccess world, BlockPos pos)
    {
        ...
        if (state.getBlock() instanceof AbstractPaintedCandleBlock)
        {
            ((AbstractPaintedCandleBlock) state.getBlock()).getParticleOffsets(state).forEach((offset) ->
            {
