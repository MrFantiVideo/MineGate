...
public class LanternBlock extends LanternRodBlock
{
    ...
    {
        super(settings);
        this.setDefaultState(this.stateManager.getDefaultState().with(TYPE, ColorsType.EMPTY).with(LIT, Boolean.FALSE).with(WATERLOGGED, Boolean.FALSE));
    }
    ...
    public ActionResult onUse(BlockState state, World world, BlockPos pos, PlayerEntity player, Hand hand, BlockHitResult hit)
    {
        ...
        if (itemStack.isIn(ItemTags.CANDLES))
        {
            ...
            {
                if (state.equals(state.with(TYPE, ColorsType.EMPTY)))
                {
                    ...
                    String candleType = item.toString().replace("_candle", "").toUpperCase();
                    ...
                    world.setBlockState(pos, state.with(TYPE, ColorsType.valueOf(candleType)));
    ...
    {
        builder.add(TYPE, UP, DOWN, EAST, NORTH, SOUTH, WEST, LIT, WATERLOGGED);
    }
    ...
    static
    {
        LANTERN_SHAPE = Stream.of(
                ...
                Block.createCuboidShape(4.0D, 4.0D, 4.0D, 12.0D, 12.0D, 12.0D)).reduce((v1, v2) ->
